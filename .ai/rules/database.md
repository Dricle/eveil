...
ADR-010. Postgres in dev, test, CI and prod. SQLite is not used anywhere: do not reintroduce it, including as a test driver.

Why Postgres: discovery runs write concurrently from several workers for minutes at a time and SQLite allows one writer at a time even in WAL. The schema is JSON-heavy (`knowledge_base`, `target_profiles.criteria`, `agent_runs.input/output`, `campaign_steps.config`) and JSONB is indexable. Dedup needs partial unique indexes; lead search will need native full-text; pgvector stays available if the knowledge base ever needs embeddings.
...
- Cloud-only credit tables (ADR-019): `credit_prices` (the per-agent grid) and `credit_transactions` (the ledger). There is no separate wallet table - the balance itself is `organizations.credits_balance`, a plain column, not a `credit_wallets` row: it was one column behind a whole model for what a wallet actually held.

## Environment precedence: two behaviours that bite in opposite directions
Both are verified empirically, not assumed. They interact, and getting them backwards produces silent wrongness rather than an error.
