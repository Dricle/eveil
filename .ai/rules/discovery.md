...
## degoog runs as a second web-search source, one probe per source
Issue #27. `DegoogSearchSource` (app/Services/Discovery/Sources/DegoogSearchSource.php) is a sibling of `WebSearchSource`, hitting degoog's `/api/search?format=json` (SearXNG-shaped `results[]`, same `toCandidate()` mapping). `Planner::probes()` emits BOTH a `web_search` and a `degoog` entry for every planned web query - two separate `RunProbe` tasks, not one task querying both. Consequence: each web query now costs 2 against `max_queries` (a map probe still costs 1); `DiscoveryPlanner`'s prompt says so. Overlap between the two sources collapses for free through the existing domain-dedupe in `DiscoveryJob::queueQualifications()` (checks existing Company + already-queued Qualify task in the run before either source's candidate is qualified) - no new dedupe code needed. degoog is off by default: optional `profiles: ['degoog']` compose service in both compose.yaml and compose.deploy.yaml, config('eveil.sources.degoog.url') defaults to the compose service DNS name. A source that isn't running fails via `ReportsFailures` like any other - never fatal to a run.

## RegistrySource: OpenRegistry proxy, and the no-website/no-email gate it needed fixed
A third discovery source, `RegistrySource`, searches official government business registries (KBO/BCE, SIRENE, Companies House, ~30 jurisdictions) through one free proxy, OpenRegistry (`openregistry.sophymarine.com/api/v1`), rather than one adapter per country. Unlike every other source it needs a key (`OPENREGISTRY_TOKEN`, free signup) - with none set it reports itself as unusable via `ReportsFailures`, same as any dead source, never crashes the run.

A registry record has a legal name, registered address and status, never an email or a site (`website: null`, no `sourceUrl` - the latter avoids `HostRegistry` ever classifying OpenRegistry's own domain as an index/directory). Live-verified response shape (2026-09-15, real token): top-level `company_id`, `company_name`, `status`, `status_detail`, `incorporation_date`, `registered_address`, `jurisdiction_data` (raw, varies wildly per country - not parsed). `q` and `jurisdiction` (ISO 3166-1 alpha-2, or a regional variant like CA-BC) are both required or the API 500s; ask jurisdiction is planned by `DiscoveryPlanner` same as an Overpass `country`.

Building this surfaced a real gate bug in `DiscoveryJob::queueQualifications()`: a no-website candidate was only kept when `facts['email']` was set, even though `Candidate`'s own docblock always promised "kept when the listing published an address" - `facts['address']` was never actually checked. Fixed to check both. Before this fix, an address-only candidate (a registry record, or any future source shaped like one) was silently discarded before ever reaching `Qualifier`, not merely scored thin - worth remembering if a new no-website source's candidates vanish with `candidates_found: 0` and no error.
