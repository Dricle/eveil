...
Eveil = **organic, automated AI marketing** for a founder or small team with no growth headcount to spare. Not a data provider, not a CRM, not a paid-ads platform on its own. Three pillars, built in this order: cold email (shipped end to end), content generation for LinkedIn and, planned, other platforms (LinkedIn shipped), and ad management (roadmap). See `GUIDELINES.md`'s "Category anchor" for the full reasoning, including why "organic" scopes the first two pillars specifically and not the third.

Positioning, stated precisely: no single competitor spans all three pillars. Lemlist, Instantly, Smartlead, Saleshandy, Reply.io cover cold email only - proprietary SaaS billed per seat and per mailbox, with prospect data hosted by the vendor. Self-hosting unlocks unlimited mailboxes at no cost (exactly what Instantly/Smartlead charge for), data sovereignty, and no per-seat billing.

[Linki](https://github.com/moaljumaa/linki) is open source and claims the LinkedIn content pillar specifically ("open source lemlist alternative", LinkedIn-first, manual targeting, effectively a lead magnet for Opsily managed hosting) - cite it honestly for that pillar rather than the whole product. Eveil's defensible slot is **the combination**: zero-config targeting derived from the product URL, driving cold email and content (and, later, ads) from one shared knowledge base.
...
- `<UApp>` wraps the ROOT in `createInertiaApp`'s `setup()` (`resources/js/app.ts`), with `app.use(ui)` there too. Never as the default `layout`: Inertia applies a default layout only to a page that declares none, and nearly every page declares `AppLayout`, so a default-layout `<UApp>` silently missed them (tooltips threw `Injection Symbol(TooltipProviderContext) not found`). Components auto-import, so `<UButton>` and friends need no import line.
