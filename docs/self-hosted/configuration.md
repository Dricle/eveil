...
Once that's saved, any organization member connects their own profile from **LinkedIn → Account** (its own item in the main navigation), grants it to whichever projects should post through it, and sets a posting cadence per project. Every draft — from the knowledge-base rotation, a client win, relevant industry news, or drafted by asking Evie — lands in **LinkedIn → Posts** for review; nothing publishes without an explicit approve.
