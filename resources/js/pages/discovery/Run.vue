...
// escape hatch, called with this page's own props each render so `current`
// tracks the run being shown, including across a persisted-layout navigation
// that never remounts TargetsLayout itself.
defineOptions({ layout: (pageProps: { run: DiscoveryRun }) => [AppLayout, [TargetsLayout, { current: pageProps.run.profile_id }]] })
