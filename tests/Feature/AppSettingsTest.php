...
it('shows what each agent has already spent', function () {
    ...
    $this->actingAs(superAdmin())->get(route('app-settings.agents.index'))
        ->assertInertia(fn ($page) => $page
            ->where('agents.15.slug', 'website-analyst')
            ->where('agents.15.calls', 2)
            ->where('agents.15.tokens_in', 200)
            ->where('agents.15.tokens_out', 100)
            ->where('agents.15.avg_tokens_in', 100)
            ->where('agents.15.avg_tokens_out', 50));
});
...
it('has no credit price for an agent nobody has priced', function () {
    ...
    $this->actingAs(superAdmin())->get(route('app-settings.agents.index'))
        ->assertInertia(fn ($page) => $page->where('agents.15.credit_price', null));
});
...
it('marks the agents that shipped calibrated to run on the cheap model', function () {
    $this->actingAs(superAdmin())->get(route('app-settings.agents.index'))
        ->assertInertia(fn ($page) => $page
            ...
            ->where('agents.15.slug', 'website-analyst')
            ->where('agents.15.smallModelOk', false));
});
